...
<style>
    ...
    getAllAvailableDocumentsRequest();

    }

    function getAllAvailableDocumentsRequest() {
    current_page = <?php echo (isset($_GET['page']) ? $_GET['page'] : 1); ?>;

    var request = $.ajax({
        type: "GET",
        // dataType:"json",
        url: "<?php echo getFullInciteUrl().'/ajax/getallavailabledocs'; ?>",
    ...
    function buildMap(){
        ...
        function addTaskCompletionIconsToResultsRow(isTranscribed, isTagged, isConnected, documentId) {
            ...
            if (isTranscribed) {
                ...
                if (isTagged) {
                    taggedIcon = $('<a href="<?php echo getFullInciteUrl(); ?>/documents/tag/' + documentId + (query_str != "" ? "?" + query_str : "") + '">' +
                        '<span title="Document has been tagged - Click to edit" class="glyphicon glyphicon-tags task-icon black-color"></span></a>');

                    if (isConnected) {
                        connectedIcon = $('<a href="<?php echo getFullInciteUrl(); ?>/documents/connect/' + documentId + (query_str != "" ? "?" + query_str : "") +'">' +
