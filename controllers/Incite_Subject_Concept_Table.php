 ...
 * @param int $userID
 */
function addConceptToDocument($conceptID, $documentID, $userID, $positive)
{
    $db = DB_Connect::connectDB();
    ...
    if($stmt->num_rows > 0) 
    {
        ...
        $newStmt->bind_param("iii", $conceptID, $id, $positive);
        $newStmt->execute();
        $newStmt->close();
    ...
    else
    {
        ...
        $newStmt1->bind_param("ii", $conceptID, $id, $positive);
        $newStmt1->execute();
        $newStmt1->close();
    }
}
